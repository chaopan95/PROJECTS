...
function EUF = CalculateExpectedUtilityFactor( I )
  ...
  %         utilities.
  % Return value: A factor over the scope of the decision rule D from I
  %         that gives the conditional utility given each assignment
  %         for D.var
  %
  ...
  all_var = unique([X(:).var]);

  F = [X, U];
  factor_product = X(1);
  for i = 2:length(F)
      factor_product = FactorProduct(factor_product, F(i));
  end;
  
  EUF = FactorMarginalization(factor_product, setdiff(all_var, D.var));
