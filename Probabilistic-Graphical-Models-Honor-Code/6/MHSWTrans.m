...
%  variant - a number (1 or 2) indicating the variant of Swendsen-Wang to
%            use.  In variant 1, all the q_{i,j}'s are equal
%
...
% Check that the dimensions are all the same and they have the same current
% assignment
...
elseif variant == 2
    ...
    V = selected_vars;
    LogR = BlockLogDistribution(V, G, F, A);
...
% Get the log-ratio of the probability of picking the connected component Y
% given A_prop over A
log_QY_ratio = 0.0;

% Iterate through *all* edges, not just the ones we selected earlier
for i = 1:size(G.q_list, 1)
    u = G.q_list(i, 1);
    v = G.q_list(i, 2);
    % the edge is from Y to outside-Y
    if length(intersect([u, v], selected_vars)) == 1
...
p_acceptance = min(1, exp(delta+ratio));
