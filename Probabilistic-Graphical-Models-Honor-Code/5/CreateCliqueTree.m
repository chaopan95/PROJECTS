...
%
%   P = CREATECLIQUETREE(F, Evidence) Takes a list of factors and creates
%   a clique tree. The value of the cliques should be initialized to the
%   initial potential. It returns a clique tree that has the following
%   fields:
...
while cliquesConsidered < length(V)
	 ...
	 % Using Min-Neighbors where you prefer to eliminate the variable that
     % has the smallest number of edges connected to it. 
...
% Here we make the function call to assign factors to cliques and compute
% the initial potentials for clusters.
