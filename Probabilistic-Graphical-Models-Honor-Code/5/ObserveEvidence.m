...
%       pair. 
%     Variables are in the first column and values are in the second
%       column.
%
    ...
    for j = 1:length(F),
        ...
        if (~isempty(indx)),
            ...
            if (x > F(j).card(indx) || x < 0 ),
                error(['Invalid evidence, X_', int2str(v), ' = ',...
                    int2str(x)]);
            end;
            ...
            if (all(F(j).val == 0)),
                warning(['Factor ', int2str(j),...
                    ' makes variable assignment impossible']);
            end;
