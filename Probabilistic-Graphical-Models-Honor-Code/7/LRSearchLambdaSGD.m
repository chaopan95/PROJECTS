% function allAcc = LRSearchLambdaSGD(Xtrain, Ytrain, Xvalidation,
% Yvalidation, lambdas). For each value of lambda provided, fit parameters
% to the training data and return the accuracy in the validation data in
% the corresponding entry of allAcc. For instance, allAcc(i) = accuracy in
% the validation set using lambdas(i).
%
% Inputs:
...
function allAcc = LRSearchLambdaSGD(Xtrain, Ytrain, Xvalidation,...
  ...
  % You may use the functions we have provided such as LRTrainSGD,
  % LRPredict, and LRAccuracy.
  ...
  for i = 1:length(lambdas)
      lambda = lambdas(i);
      thetaOpt = LRTrainSGD(Xtrain, Ytrain, lambda);
