% [cost, grad] = LRCostSGD(X, y, theta, lambda, i) calculates the LR
% cost / objective function with respect to data instance (i mod n), given
% the LR classifier parameterized by theta and where n = the number of data
% instances.  Also returns the gradient of the cost function with respect
% to data instance (i mod n). The aim of LR training is to find the theta
% that minimizes this function.
%
% Inputs:
% X         data.                           (numInstances x numFeatures
%                                            matrix)
...
function [cost, grad] = LRCostSGD(X, y, theta, lambda, i)
    ...
    cost = sum((-y(i) .* log(h)) - ((1 - y(i)) .* log(1 - h))) +...
