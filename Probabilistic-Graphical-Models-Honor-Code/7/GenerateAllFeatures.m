...
function featureSet = GenerateAllFeatures(X, modelParams)
    ...
    unconditionedFFs = {@ComputeUnconditionedSingletonFeatures,...
        @ComputeUnconditionedPairFeatures};
    ...
    for i = 1:numUnconditionedFFs
        if (~isa(unconditionedFFs{i}, 'function_handle'))
            error(['Non-function variable in unconditionedFFs ',...
                'argument to GenerateAllFeatures']);
        end
    end
    for i = 1:numConditionedFFs
        if (~isa(conditionedFFs{i}, 'function_handle'))
            error(['Non-function variable in conditionedFFs argument ',...
                'to GenerateAllFeatures']);
        end
    ...
    if (numConditionedFFs > 0)
        prevMaxParam =...
            NumParamsForConditionedFeatures(conditionedCell{1},...
            modelParams.numObservedStates);
    else
        prevMaxParam = 0;
    end

    for i = 2:1:numConditionedFFs
        conditionedCell{i} = IncrementFeatureParams(conditionedCell{i},...
            prevMaxParam);
    ...
    for i = 1:1:numUnconditionedFFs
        unconditionedCell{i} =...
            IncrementFeatureParams(unconditionedCell{i}, prevMaxParam);
        prevMaxParam =...
            NumParamsForUnconditionedFeatures(unconditionedCell{i});
    end
