...
genotypeVar = 1;
phenotypeVar = 3;
phenotypeFactor = struct('var', [3,1], 'card', [2,3], 'val',...
    [1,0,1,0,0,1]);
% Comment out this line for testing
phenotypeFactor = phenotypeGivenGenotypeMendelianFactor(isDominant,...
...
genotypeVar = 1;
phenotypeVar = 3;
phenotypeFactorAlpha = struct('var', [3,1], 'card', [2,3],...
    'val', [0.8,0.2,0.6,0.4,0.1,0.9]);
% Comment out this line for testing
phenotypeFactorAlpha = phenotypeGivenGenotypeFactor(alphaList,...
    genotypeVar, phenotypeVar);
...
genotypeFactorPar = struct('var', [3,1,2], 'card', [3,3,3],...
    'val', [1,0,0,0.5,0.5,0,0,1,0,0.5,0.5,0,0.25,0.5,0.25,0,0.5,0.5,...
    0,1,0,0,0.5,0.5,0,0,1]);
% Comment out this line for testing
genotypeFactorPar = genotypeGivenParentsGenotypesFactor(numAlleles,...
...
alleleFreqs = [0.1; 0.9];
alphaList = [0.8; 0.6; 0.1];
sampleFactorList = load('sampleFactorList.mat');
% Comment out this line for testing
...
phenotypeVar = 3;
phenotypeFactorPar = struct('var', [3,1,2], 'card', [2,3,3],...
    'val', [0.8,0.2,0.6,0.4,0.1,0.9,0.6,0.4,0.5,0.5,0.05,0.95,0.1,...
    0.9,0.05,0.95,0.01,0.99]);
% Comment out this line for testing
phenotypeFactorPar = phenotypeGivenCopiesFactor(alphaListThree,...
    numAllelesThree, genotypeVarMotherCopy, genotypeVarFatherCopy,...
    phenotypeVar);
...
% Comment out this line for testing
sampleFactorListDecoupled = constructDecoupledGeneticNetwork(pedigree,...
    alleleFreqsThree, alphaListThree);
...
phenotypeVar = 3;
phenotypeFactorSigmoid = struct('var', [3,1,2,4,5], 'card', [2,2,2,2,2],...
    'val', [0.999590432835014,0.000409567164986080,0.858148935099512,...
    0.141851064900488,0.997762151478724,0.00223784852127629,...
    0.524979187478940,0.475020812521060,0.858148935099512,...
    0.141851064900488,0.0147740316932731,0.985225968306727,...
    0.524979187478940,0.475020812521060,0.00273196076301106,...
    0.997268039236989,0.997762151478724,0.00223784852127629,...
    0.524979187478940,0.475020812521060,0.987871565015726,...
    0.0121284349842742,0.167981614866076,0.832018385133925,...
    0.524979187478940,0.475020812521060,0.00273196076301106,...
    0.997268039236989,0.167981614866076,0.832018385133925,...
    0.000500201107079564,0.999499798892920]);
% Comment out this line for testing
phenotypeFactorSigmoid = constructSigmoidPhenotypeFactor(alleleWeights,...
    geneCopyVarParentOneList, geneCopyVarParentTwoList, phenotypeVar);
