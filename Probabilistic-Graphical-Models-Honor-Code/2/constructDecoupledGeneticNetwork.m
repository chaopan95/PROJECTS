function factorList = constructDecoupledGeneticNetwork(pedigree,...
    alleleFreqs, alphaList)
...
% 1 variable number i.  If the parents are specified, then gene copy 1 is
% the copy inherited from pedigree.names{pedigree.parents(i, 1)}.
%
% 2.  The next n variables should be the gene copy 2 variables and should
% be numbered according to the index of the corresponding person in
% pedigree.names; the ith person with name pedigree.names{i} has gene copy
% 2 variable number n+i.  If the parents are specified, then gene copy 2 is
% the copy inherited from pedigree.parents(i, 2).
%
...
for i = 1:numPeople
    if pedigree.parents(i, 1) == 0
        geneCopyVar = i;
        factorList(i) = childCopyGivenFreqsFactor(alleleFreqs,...
            geneCopyVar);
        geneCopyVar = i + numPeople;
        factorList(i + numPeople) = childCopyGivenFreqsFactor(...
            alleleFreqs, geneCopyVar);
    else
        geneCopyVarChild = i;
        ...
        factorList(i) = childCopyGivenParentalsFactor(numAlleles,...
            geneCopyVarChild, geneCopyVarOne, geneCopyVarTwo);
        geneCopyVarChild = i + numPeople;
        geneCopyVarOne = pedigree.parents(i, 2);
        geneCopyVarTwo = pedigree.parents(i, 2) + numPeople;
        factorList(i + numPeople) = childCopyGivenParentalsFactor(...
            numAlleles, geneCopyVarChild, geneCopyVarOne, geneCopyVarTwo);
    end;

    geneCopyVarOne = i;
    geneCopyVarTwo = i + numPeople;
    phenotypeVar = i + 2*numPeople;
