...
phenotypeFactor.var = [phenotypeVar geneCopyVarOneList'...
    geneCopyVarTwoList'];

% only 2 genes
num_gene = length(alleleWeights);
num_allele = length(alleleWeights{1});

phenotypeFactor.card = [num_allele,...
    num_allele*ones(1, length(geneCopyVarOneList')),...
    num_allele*ones(1, length(geneCopyVarTwoList'))];
...
for k = 1:prod(phenotypeFactor.card)
    phenotype = assignments(k, 1);
    f = 0;
    for j = 1:num_gene
        for i = 1:num_allele
            X = 0;
            Y = 0;
            if assignments(k, j+1) == i
                X = 1;
            end;
            if assignments(k, j+1+length(geneCopyVarOneList)) == i
                Y = 1;
            end;
            f = f + alleleWeights{j}(i)*(X + Y);
        end;
    end;
