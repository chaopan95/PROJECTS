...
P.cliqueList = repmat(struct('var', [], 'card', [], 'val', []),...
    numNodes, 1);
...
for i = 1:length(F)
    ...
    if (length(f.var) == 1)
        ...
        for assignment = 1:card
           if (f.var == cliqueIdx)
               updateIdxs = AssignmentToIndex(...
                   [(1:card)' assignment*ones(card,1)], [card card]);
           else
               updateIdxs = AssignmentToIndex(...
                   [assignment*ones(card,1) (1:card)'], [card card]);
           end
           P.cliqueList(cliqueIdx).val(updateIdxs) =...
               P.cliqueList(cliqueIdx).val(updateIdxs) + f.val;
        end        
    else
