...
model = ones(K, K, K);
for num = 1:length(tripletList)
    i = tripletList(num).chars(1);
    j = tripletList(num).chars(2);
    k = tripletList(num).chars(3);
    model(i, j, k) = tripletList(num).factorVal;
end;
...
for i = 1:n-2
    ...
    factors(i).val = reshape(model, K*K*K, 1);
end;
